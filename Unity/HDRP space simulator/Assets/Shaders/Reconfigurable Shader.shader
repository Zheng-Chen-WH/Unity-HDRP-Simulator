Shader "Planet/PlanetShader"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			uniform fixed _SpecularIntensity;		uniform fixed _OceanShininess;			uniform fixed _NightIntensity;
...
fixed specularMask = tex2D(_SpecularTex, i.uv).a;
                
                // 使用 specular texture 作为海洋遮罩，如果没有则用颜色亮度判断
                float oceanMask = specularMask > 0.01 ? specularMask : 
                                  (1.0 - saturate(dot(dayColor.rgb, float3(0.3, 0.59, 0.11))));
                oceanMask = pow(oceanMask, 2) * _OceanShininess;
                ...
                fixed specTerm = pow(nh, _SpecularPower) * _Smoothness * oceanMask * d;

				fixed4 result;
				fixed3 sunIntensity = sunColor * d;
			fixed3 dayColorSpec = (dayColor * sunIntensity * e) + (sunColor * specTerm * _SpecularIntensity);
